...
#### 2、TCP协议如何保证可靠传输
+ 通过对每个TCP报文加上序列号，利用滑动窗口来缓存已发送未确认的报文（发送端），ACK确认和超时重传机制来保证数据报文传递的可靠性、有序性
...
+ IO多路复用：同时监听多个IO通道，通过调用SELECT、EPOLL函数，阻塞应用线程，直到监听的IO通道有数据就绪，同样数据拷贝过程，应用也是阻塞的

+ 信号驱动IO：对监听的IO通道，安装一个信号处理函数。当有数据准备好时，生成一个信号发送给应用，通知应用调用IO函数去处理数据，相比IO多路复用，免去了等待有数据可读或可写之前的阻塞等待，但是对于数据拷贝过程来说，应用同样也是阻塞的。

+ 异步IO：通过系统调用告诉内核在IO通道有数据可读可写时，执行某个操作，并且让内核执行完整个操作后通知应用进程/线程，最主要的特征是在内核在进行数据拷贝时，应用不阻塞，数据拷贝完成后才通知应用
...
#### 11、Netty的线程模型是怎么样的
+ 采用`Reactor`线程模型，通过配置启动参数，可以支持`Refactor`单线程模型、`Refactor`多线程模型、主从`Refactor`多线程模型。Netty服务端在启动时，创建两个独立的线程池（`NioEventLoopGroup`），一个线程池负责接受客户端的`TCP`连接，另一个线程池负责处理`I/O`相关的读写操作。此外，为了避免由于多个IO线程并发操作同一个`channel`导致并发问题，或者引入同步机制导致处理更加复杂，采用了每个`channel`绑定到一个特定的线程上，此后该线程负责此`channel`的读写事件处理。线程与绑定的`channel`之间是一对多关系，即一个线程上可以绑定多个`channel`，但是一个`channel`不能绑定到多个线程上
...
#### 14、Netty的对象池是怎么实现的
+ 将每一个缓存的对象包装成为一个`DefaultHandle`类对象，利用`Stack`对象存储回收的对象，以及获取对象，回收和获取分别对应于`push()`、`pop()`操作。`Stack`对象包含了一个本地的对象数组，和一个`WeakOrderQueue`对象组成的链表，其中本地对象数组存储了本线程回收的对象，`WeakOrderQueue`链表中的每一个节点存储了其他的一个线程回收到该`Stack`中的对象。当取对象时，优先从本地对象数组中取，如果本地对象数组中取不到对象，则从`WeakOrderQueue`链表中优先获取head `WeakOrderQueue`的head `Link`中的全部对象，置换到本地对象数组。(每个`WeakOrderQueue`都对应着一个`Link`类型的链表 ，每个`Link`节点都包含一个对象数组，用于本地对象数组为空时，从`WeakOrderQueue`中获取`DefaultHandle`，迁移到`Stack`中的本地对象数组。)。
