# 个人技术栈学习文档

+ [Redis学习笔记](./Redis知识点总结.MD)

+ [JVM垃圾回收相关.MD](./JVM/JVM相关知识点（JDK8及其以上）.MD)

+ [Kafka使用与底层实现](./Kafka/Kafka知识点.MD)

### **Netty**

+ [Netty——常用知识点](./Netty/Netty常用知识点.MD)

+ [Netty——ByteBuffer与ByteBuf区别](./Netty/ByteBuffer与ByteBuf区别.MD)

+ [Netty——解决粘包和拆包问题的四种方案](./Netty/Netty解决粘包和拆包问题的四种方案.MD)

+ [Netty——为什么Netty使用NIO而不是AIO](./Netty/为什么Netty使用NIO而不是AIO.MD)

### **Spring**

+ [Spring基础知识](./Spring/Spring基础.MD)

+ [SpringMVC使用](./Spring/SpringMVC.MD)

+ [Spring高级知识](./Spring/Spring高级知识点.MD)

### **MySQL**

+ [MySQL复制机制](./架构设计/【数据库】MySQL复制机制.MD)

+ [MySQL如何解决读写分离下的数据一致性](./架构设计/【数据库】如何解决读写分离的一致性问题.MD)

### **Java多线程**

+ [Java多线程之sychronized](./常用书签/Blog/Java多线程之sychronized.MD)

+ [Java多线程之ThreadLocal](./常用书签/Blog/Java多线程之ThreadLocal.MD)

+ [交替打印奇偶数的常见实现方式](./常用书签/Blog/交替打印奇偶数的常见实现方式.MD)

### **设计模式**

+ [单例模式的五种实现](./常用书签/Blog/单例模式的五种实现.MD)

+ [委托模式的简单实现](./常用书签/Blog/委托模式的简单实现.MD)

+ [代理模式的简单实现](./常用书签/Blog/代理模式的简单实现.MD)

+ [适配器模式的简单实现](./常用书签/Blog/适配器模式的简单实现.MD)

+ [模板方法模式的简单实现](./常用书签/Blog/模板方法模式的简单实现.MD)

### **`Dubbo`框架分析**

+ [Dubbo拓展机制使用详解](./Dubbo/Dubbo拓展机制使用详解.MD)

### **架构设计**

+ [布隆过滤器](./布隆过滤器.MD)

+ [限速器之Guava RateLimiter实现原理浅析](./常用书签/Blog/限速器之Guava%20RateLimiter.MD)

+ [Hystrix工作流程](./微服务/Hystrix工作流程.png)

+ [HystrixCircuitBreaker原理](./微服务/HystrixCircuitBreaker原理.png)

+ [服务端安全之如何防止接口盗刷](./架构设计/【防盗刷】如何防止接口盗刷.MD)

+ [分布式一致性协议与Zookeeper](./Zookeeper/Zookeeper知识点.MD)

+ [分布式一致性协议之Raft](./Raft/分布式一致性协议之Raft.MD)