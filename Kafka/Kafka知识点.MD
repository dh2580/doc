...
执行过日志压缩后的日志分段文件要比之前的未压缩钱的日志分段文件要小，为了防止出现大量的小文件，在清理过程会所有的日志文件进行分组，每个组内的所有日志分段大小之和不超过`broker`端`log.segment.bytes`参数配置，按照组进行日志压缩处理，每个组压缩后只会产生一个压缩后的日志文件，如下图所示
