...
## 25、消息日志、消息索引分割
消息日志、消息索引是以段的形式存在，并且消息日志段、索引段都是一起存在，比如存在一个`{baseOffset}.log`的日志段，必然存在`{baseOffset}.index`、`{baseOffset}.timeindex`的两个文件，**三个文件的`{baseOffset}`一致**，只要满足以下四个条件之一都会触发日志/索引分段操作：
